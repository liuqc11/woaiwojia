...
def objective(pop, keys, collection):
    ...
    for subset in subsets:
        if len(subset['jjj'])<min_length or len(subset['jzj'])<min_length:
            objvalue=0
        else:
            ...
            objvalue=abs(meanjjj-meanjzj)/(stdjjj+stdjzj)
...
#                    print keys[m_in]
            ...
            best_data_keys=['简装房数目','精装房数目','简装价均值','精装价均值','简装价方差','精装价方差']
            # '精装价方差','简装周期均值','精装周期均值','简装周期方差','精装周期方差']
...
def get_subsets_by_pops(pop, keys, collection):
    ...
    subset=[{'jzj':[],'jjj':[]} for i in range(popsize)]
        ...
        for index in range(data_num):
            ...
            if ok:
                if collection[u'装修'][index]==0:
                    ...
                    subset[pop_index]['jjj'].append(collection[u'均价'][index])
                elif collection[u'装修'][index]==1:
                    # 精装
                    # subset[pop_index]['jzzq'].append(collection[u'成交周期'][index])
                    subset[pop_index]['jzj'].append(collection[u'均价'][index])
    return subset
